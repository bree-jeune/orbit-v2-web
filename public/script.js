...
audioLoader.load("/sounds/envato_sfxgen_Dec_23_2025_10_03_13 (1).mp3", (buffer) => {
...
function animate() {
  ...
  planets.forEach((pl, idx) => {
    ...
    pl.position.set(
      Math.cos(pl.userData.angle) * pl.userData.distance,
      Math.sin(pl.userData.angle * 0.5) * 10,
      Math.sin(pl.userData.angle) * pl.userData.distance
    );
